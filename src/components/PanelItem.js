...
export default function PanelItem(props) {
  ...
  let containerStyles = [
    styles.container,
    wrapper && styles.containerWrapper,
    getStyle(props),
  ];
  ...
  let infoStyles = [
    styles.info,
    horizontal ? { marginHorizontal: 8 } : { marginTop: 0 },
    wrapper && !horizontal && { padding: 12 },
    infoStyle,
  ];
      ...
      >
        <View style={wrapperStyles}>
          ...
          <View style={infoStyles}>
            {props.context && (
              <Text style={contextStyles} numberOfLines={1}>
                {props.context}
              </Text>
            )}

            {props.title && (
              <Text style={titleStyles} numberOfLines={horizontal ? 2 : 3}>
                {props.title}
              </Text>
            )}
            ...
            <View style={{ flexDirection: 'row', paddingVertical: 4 }}>
              {props.subtext && (
                <Text style={subtextStyles} numberOfLines={2}>
                  {props.subtext}
                </Text>
              )}
              <View style={{ flex: 1 }} />
